...
describe("GPUComputeService", () => {
  ...
  describe("updatePositionsBatch", () => {
    it("debe usar CPU para < 10 entidades", () => {
      ...
      const result = service.updatePositionsBatch(
        positions,
        targets,
        speeds,
        fatigue,
    ...
    it("debe calcular nuevas posiciones correctamente", () => {
    ...
    it("debe detectar arrived correctamente", () => {
  ...
  describe("applyNeedsDecayBatch", () => {
    it("debe aplicar decay con multiplicadores", () => {
      ...
      const result = service.applyNeedsDecayBatch(
        needs,
        decayRates,
        ageMultipliers,
        divineModifiers,
    ...
    it("debe usar CPU fallback para < 10 entidades", () => {
  ...
  describe("applyNeedsCrossEffectsBatch", () => {
    it("debe aplicar efectos cruzados entre needs", () => {
      const needs = new Float32Array([30, 30, 20, 50, 50, 50, 50]); // 1 entidad, 7 necesidades
      const needCount = 7;

      const result = service.applyNeedsCrossEffectsBatch(needs, needCount);
  ...
  describe("updateFatigueBatch", () => {
    it("debe incrementar/decrementa fatiga", () => {
      const fatigue = new Float32Array([50, 50]);
      const isMoving = [true, false];
      const isResting = [false, true];

      const result = service.updateFatigueBatch(fatigue, isMoving, isResting, 1000);
