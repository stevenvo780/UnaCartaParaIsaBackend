...
export class SimulationRunner {
  ...
  private persistenceWorkerReady = false;
  private isSaving = false;
  private lastSaveTime = Date.now();
  private readonly AUTO_SAVE_INTERVAL_MS = 60 * 1000; // 1 minute
  ...
  constructor(
    ...
    this.initializePersistenceWorker();
  }
  ...
  /**
   * Initializes a worker thread for off-main-thread game state persistence.
   *
   * The worker handles the heavy JSON serialization of the full game state,
   * preventing the main simulation loop from blocking during saves.
   */
  private initializePersistenceWorker(): void {
    try {
      const workerCode = `
        const { parentPort } = require('worker_threads');
        if (!parentPort) throw new Error('PersistenceWorker inline requires parentPort');
        parentPort.postMessage({ type: 'ready' });
        parentPort.on('message', (msg) => {
          try {
            if (msg && msg.type === 'save' && msg.data) {
              const { state, metadata } = msg.data;
              const saveData = {
                ...metadata,
                state: state
              };
              const serialized = JSON.stringify(saveData, null, 2);
              parentPort.postMessage({ type: 'save-ready', data: serialized, timestamp: metadata.timestamp });
            } else if (msg && msg.type === 'shutdown') {
              process.exit(0);
            }
          } catch (err) {
      ...
      this.persistenceWorker = new Worker(workerCode, { eval: true });

      this.persistenceWorker.on("message", async (rawMessage: unknown) => {
        const message = rawMessage as {
          type: string;
          data?: string;
          timestamp?: number;
        ...
        if (message.type === "ready") {
          this.persistenceWorkerReady = true;
          logger.info("💾 Persistence worker thread ready");
        } else if (message.type === "save-ready" && message.data && message.timestamp) {
          try {
            await this.storageService.saveSerializedGame(message.data, message.timestamp);
            logger.info("✅ Game saved successfully (async)", { timestamp: message.timestamp });
          } catch (err) {
            logger.error("Failed to write saved game to storage:", err);
          } finally {
            this.isSaving = false;
          }
        } else if (message.type === "error") {
          logger.error("Persistence worker error:", message.error);
          this.isSaving = false;
        }
      });

      this.persistenceWorker.on("error", (error) => {
        logger.error("Persistence worker thread error:", error);
        this.persistenceWorkerReady = false;
        this.isSaving = false;
      });

      this.persistenceWorker.on("exit", (code) => {
        if (code !== 0) {
          logger.warn(`Persistence worker stopped with exit code ${code}`);
        }
        this.persistenceWorkerReady = false;
        this.isSaving = false;
      });
    } catch (error) {
      logger.error("Failed to initialize persistence worker:", error);
      this.persistenceWorkerReady = false;
    }
  }

  /**
   * Triggers a non-blocking game save.
   *
   * @param type - Type of save ('auto' or 'manual')
   */
  public async saveGame(type: "auto" | "manual" = "manual"): Promise<void> {
    if (this.isSaving) {
      logger.warn("Save already in progress, skipping", { type });
      return;
    }

    if (!this.persistenceWorkerReady || !this.persistenceWorker) {
      logger.warn("Persistence worker not ready, cannot save asynchronously");
      return;
    }
    ...
    logger.info(`💾 Starting ${type} save...`);
    ...
    const metadata = {
      ...
      stats: {
        ...
        resources: this.state.worldResources ? Object.keys(this.state.worldResources).length : 0,
        cycles: this.tickCounter,
      },
    };

    // Send to worker
    // We clone the state to avoid race conditions if the state is mutated while serialization happens
    // However, structuredClone or JSON.parse(JSON.stringify) is also expensive.
    // For now, we pass the state directly. The worker serialization might race with updates,
    // but since it's a snapshot in time and we are in a single threaded JS environment (mostly),
    // passing the object to postMessage will clone it using the structured clone algorithm.
    // This cloning happens on the main thread but is generally faster than JSON.stringify.
    // If it blocks too much, we might need to use SharedArrayBuffer or other techniques,
    // but for now this is a significant improvement over JSON.stringify on main thread.
    this.persistenceWorker.postMessage({
      type: "save",
      data: {
        state: this.state,
        metadata,
...
// Auto-save check
const now = Date.now();
if (now - this.lastSaveTime >= this.AUTO_SAVE_INTERVAL_MS) {
  this.saveGame("auto").catch((err) => {
    logger.error("Auto-save failed trigger:", err);
  });
}
      },
postTick: () => {
...
while (this.commands.length > 0) {
  ...
  switch (command.type) {
    ...
    case "TIME_COMMAND":
      ...
      break;
    case "SAVE_GAME":
      this.saveGame(command.payload?.type ?? "manual");
      break;
