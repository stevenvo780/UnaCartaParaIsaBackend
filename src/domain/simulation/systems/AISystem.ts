  ...
  private processGoals(
    ...
    const deps: AgentGoalPlannerDeps = {
      ...
      findNearestHuntableAnimal: (entityId: string) => {
        ...
        let nearest: { id: string; x: number; y: number; type: string } | null = null;
      ...
      taskSystem: this.taskSystem
        ? {
          createTask: (params: unknown) => this.taskSystem!.createTask(params),
      ...
      sharedKnowledgeSystem: this.sharedKnowledgeSystem
        ? {
          getKnownResourceAlerts: (agentId: string) =>
            this.sharedKnowledgeSystem!.getKnownResourceAlerts(agentId),
          getKnownThreatAlerts: (agentId: string) =>
