...
export interface CombatContext {
  ...
  getNearbyPredators: (pos: { x: number; y: number }, range: number) => Array<{ id: string; position: { x: number; y: number } }>;
}
    ...
    if (myPos) {
        ...
        if (enemies.length > 0) {
            ...
            if (nearest) {
                ...
                const DANGER = 90 * (morale < panicThreshold ? 1.6 : 1.0) * (1 + personality.neuroticism * 0.5);
                const SEEK = 180 * (morale > 60 ? 1.0 : 0.8) * (1 + (1 - personality.agreeableness) * 0.5);
                ...
                if (!isWarrior && nearest.d < DANGER) {
                    ...
                    const fleePos = { x: myPos.x + dx * scale, y: myPos.y + dy * scale };
                    ...
                    if (strategy === "peaceful") {
                        ...
                        const aggressionThreshold = 0.9 - (1 - personality.agreeableness) * 0.2;
